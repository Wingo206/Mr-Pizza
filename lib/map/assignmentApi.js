...
async function optimalAssignment(req, res, jwtBody, storeId) {
   ...
   if (!decodedData.hasOwnProperty('drivers')) {
...
async function assignOrders(req, res, jwtBody, storeId) {
   ...
   let decodedData = JSON.parse(body);
   if (!decodedData.hasOwnProperty('orders') || !decodedData.hasOwnProperty('eid')) {
   ...
   if (checkRes.length != 0) {
      res.writeHead(400, { 'Content-type': 'text/plain' })
      res.end('Driver is already assigned a delivery batch');
      return;
   }
