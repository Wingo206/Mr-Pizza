...
async function getUnassignedOrders(req, res, jwtBody, storeId) {
   ...
   // console.log(JSON.stringify(routeMat))
...
async function assignOrders(req, res, jwtBody, storeId) {
   ...
   let storeLocation = await queryStoreLatlng(storeId);
   console.log(storeLocation)
   let batchRes = await runQuery(employeePool,
      `insert into delivery_batch(driver_status, assignedToEmp, current_latlng) values ("idle", ${eid}, POINT(${storeLocation.x}, ${storeLocation.y}))`);
