...
async function editAvailability(employeeRequest) {
    ...
    const real_available = available_switch(available);
    try {
        if (object == "item") {
            ...
            await runQuery(employeePool, `UPDATE mrpizza.custom_availability SET available = '${real_available}' WHERE store_id = '${storeId}' AND mid = '${mid}' AND option_name = '${option_name}' AND custom_name = '${object}'`);
        }
    } catch (error) {
        throw error;
    }
    ...
    try {
        if (action == 'delete') {
            if (object == 'item') {
                ...
                await runQuery(adminPool, `DELETE FROM mrpizza.custom WHERE mid = '${mid}' AND option_name = '${name}' AND custom_name = '${object}'`);
            }
        } else { // Edit object
            if (object == 'item') {
                ...
                if (property == 'option_name') {
                    await runQuery(adminPool, `UPDATE mrpizza.custom_option SET ${property} = '${newValue}' WHERE mid = '${mid}' AND option_name = '${name}' AND custom_name = '${object}'`);
                    await runQuery(adminPool, `UPDATE mrpizza.custom_availability SET ${property} = '${newValue}' WHERE mid = '${mid}' AND option_name = '${name}' AND custom_name = '${object}'`);
                } else {
                    await runQuery(adminPool, `UPDATE mrpizza.custom_option SET ${property} = '${newValue}' WHERE mid = '${mid}' AND option_name = '${name}' AND custom_name = '${object}'`);
                }
            }
...
function available_switch(available) {
...
async function addCustom(adminRequest) {
    ...
    const storeId = 1;
    try {
        await runQuery(adminPool, `INSERT INTO mrpizza.custom_option (custom_name, mid, option_name, price, isDefault) VALUES ('${custom_name}', '${mid}', '${option_name}', '${price}', '${isDefault}')`);
        await runQuery(adminPool, `INSERT INTO mrpizza.custom_availability (mid, custom_name, option_name, store_id, available) VALUES ('${mid}', '${custom_name}', '${option_name}', '${storeId}', '${available}')`);
        await runQuery(adminPool, `INSERT INTO mrpizza.custom (custom_name, mid, mutually_exclusive) VALUES ('${custom_name}', '${mid}', '${mutually_exclusive}')`);
    } catch (error) {
        throw error;
    }
}
