...
async function getMenuItemsFromDatabase(storeId) {
    try {
        ...
        const queryItems = await runQuery(customerPool, `SELECT * FROM mrpizza.menu_item JOIN mrpizza.item_availability USING (mid) WHERE store_id = ${storeId};`);
        const queryToppings = await runQuery(customerPool, `SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = ${storeId} AND custom_name = 'toppings';`);
        const querySize = await runQuery(customerPool, `SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = ${storeId} AND custom_name = 'size';`);
        const queryServingOptions = await runQuery(customerPool, `SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = ${storeId} AND custom_name = 'servingOptions';`);
        const querySauces = await runQuery(customerPool, `SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = ${storeId} AND custom_name = 'sauces';`);
        const queryCrust = await runQuery(customerPool, `SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = ${storeId} AND custom_name = 'crust';`);
        const queryMeats = await runQuery(customerPool, `SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = ${storeId} AND custom_name = 'meats';`);
        const queryNonMeats = await runQuery(customerPool, `SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = ${storeId} AND custom_name = 'nonMeats';`);
        const queryCheese = await runQuery(customerPool, `SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = ${storeId} AND custom_name = 'cheese';`);
