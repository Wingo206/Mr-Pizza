...
async function getMenuItemsFromDatabase(storeId) {
    try {
        ...
        const queryItems = await runQuery(customerPool,'SELECT * FROM mrpizza.menu_item JOIN mrpizza.item_availability USING (mid) WHERE store_id = 1 AND available = 1;');
        const queryToppings = await runQuery(customerPool,"SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = 1 AND available = 1 AND custom_name = 'topping';");
        const querySize = await runQuery(customerPool,"SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = 1 AND available = 1 AND custom_name = 'size';");
