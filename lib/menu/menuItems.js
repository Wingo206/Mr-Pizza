...
async function getMenuItemsFromDatabase(storeId) {
    try {
        ...
        const queryItems = await runQuery(customerPool, `SELECT * FROM mrpizza.menu_item JOIN mrpizza.item_availability USING (mid) WHERE store_id = ${storeId} AND available = '1';`);
        const queryToppings = await runQuery(customerPool, `SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = ${storeId} AND custom_name = 'toppings' AND available = '1';`);
        const querySize = await runQuery(customerPool, `SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = ${storeId} AND custom_name = 'size' AND available = '1';`);
        const queryServingOptions = await runQuery(customerPool, `SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = ${storeId} AND custom_name = 'servingOptions' AND available = '1';`);
        const querySauces = await runQuery(customerPool, `SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = ${storeId} AND custom_name = 'sauces' AND available = '1';`);
        const queryCrust = await runQuery(customerPool, `SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = ${storeId} AND custom_name = 'crust' AND available = '1';`);
        const queryMeats = await runQuery(customerPool, `SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = ${storeId} AND custom_name = 'meats' AND available = '1';`);
        const queryNonMeats = await runQuery(customerPool, `SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = ${storeId} AND custom_name = 'nonMeats' AND available = '1';`);
        const queryCheese = await runQuery(customerPool, `SELECT * FROM (mrpizza.custom_option JOIN mrpizza.custom_availability USING (custom_name, mid, option_name)) JOIN mrpizza.custom USING (custom_name, mid) WHERE store_id = ${storeId} AND custom_name = 'cheese' AND available = '1';`);
...
/**
 * Handles requests for fetching all menu items
 * First digit: store id
 */
async function handleMenuItem(req, res) {
    let nums = req.url.match(/\d+/g);
    let storeId = nums[0];

    // make sure store id is a valid store
    let storeRes = await runQuery(customerPool, `SELECT * FROM store WHERE store_id = ?`, storeId);
    ...
    try {
        const menuItems = await getMenuItemsFromDatabase(storeId);
