const { runQuery, authPool, adminPool } = require('./util/database_util');
...
async function getMenuItemsFromDatabase(storeId) {
    try {
        ...
        const queryItems = await runQuery(adminPool,'SELECT * FROM menu_Item JOIN item_availability USING (mid) WHERE store_id = "${storeId}" AND available = 1;');
        const queryToppings = await runQuery(adminPool,'SELECT * FROM topping JOIN topping_availability USING (topping_name, mid) WHERE store_id = "${storeId}" AND available = 1;');
