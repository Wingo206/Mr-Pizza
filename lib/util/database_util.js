...
async function resetDatabase() {
   ...
   for (let i = 0; i < tablesInfo.length; i++) {
      let entry = tablesInfo[i];
      await runQuery(adminPool, `delete from ${entry.TABLE_NAME}`);
      if (entry.AUTO_INCREMENT != null) {
         await runQuery(adminPool, `alter table ${entry.TABLE_NAME} AUTO_INCREMENT = 1`);
      }
   }
