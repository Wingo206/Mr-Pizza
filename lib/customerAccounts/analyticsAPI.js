...
async function getEmployeeCountByStore(req, res) {
    try {
        // Modified to LEFT JOIN to include stores with no employees and corrected GROUP BY clause
        const query = `
        ...
        // Check if there are results before attempting to access them
        if (!result || !result.rows || result.rows.length === 0) {
            res.writeHead(404, { 'Content-type': 'application/json' });
            res.end(JSON.stringify({ error: "No data found" }));
            return;
        ...
        // Check if headers have already been sent to handle multiple responses
        if (res.headersSent) {
            console.error('Headers already sent:', err);
            res.end(JSON.stringify({ error: err.message })); // Only end the response if not already ended
        } else {
