...
//    });
   ...
   const customerOrderQuery = 'INSERT INTO customer_order (made_at, credit_card, status, total_price, delivery_address, DT_created, DT_delivered, ordered_by) VALUES ?';
   let orderId = 0;
...
async function getOrderID(req, res) {
   ...
   let decodedData = JSON.parse(body);
   let customer_id = decodedData.customer_id;
   ...
   try {
      ...
      console.error("Error:", error.message);
      res.writeHead(500, {'Content-type': 'text/plain'});
      res.end("Error retrieving order ID of customer " + customer_id + ".");
   }
}
