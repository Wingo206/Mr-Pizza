...
async function handleOrder(req, res, jwtBody) {
    ...
    res.write(JSON.stringify({ orderId: orderId, message: 'Order ' + orderId + ' confirmed' }));
    res.end();
}
...
//Create review for customers order_item
async function createReview(req, res, jwtBody) {
   ...
   req.on('end', async () => {
       ...
       try {
           ...
           let { description, item_num } = decodedData;

           const insertQuery = `UPDATE order_item SET reviewDescription = "` + description + `" WHERE item_num = ` + item_num;
           await runQuery(customerPool, insertQuery);
...
async function getDescription(req, res) {
   ...
   req.on('end', async () => {
       ...
       try {
           const { item_num } = JSON.parse(body);
           const insertQuery = `SELECT reviewDescription FROM order_item WHERE item_num = ` + item_num;
           let result = await runQuery(customerPool, insertQuery);

           res.writeHead(201, { "Content-Type": "text/plain" });
           res.end(JSON.stringify(result[0].reviewDescription));
       } catch (error) {
           console.error(error);
           res.writeHead(500, { "Content-Type": "text/plain" });
           res.end("Internal server error.");
...
module.exports = {
   routes: [
      ...
      },
      {
         ...
         handler: handleAuth(authRoles.customer, createReview)
      },
      {
         method: "POST",
         path: '/order/getReview',
         handler: handleAuth(authRoles.customer, getDescription)
      }
   ]
};
