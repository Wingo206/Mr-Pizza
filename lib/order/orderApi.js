...
//    });
   ...
   if (!validation.valid) {
      res.writeHead(400, {'Content-type': 'text/plain'});
      res.end('Invalid Formatted Address');
      return;
   }
   ...
   const customerOrderQuery = 'INSERT INTO customer_order (made_at, credit_card, status, total_price, delivery_address, delivery_latlng, DT_created, DT_delivered, ordered_by) VALUES ?';
   let orderId = 0;
   ...
   try {
      result = runQuery(customerPool, orderItemQuery, [ orderItemValues ]);
   } catch {
   ...
   console.log('Inserted into order_item:', result);
      ...
      res.write('Order ' + orderId + ' confirmed, inserted into database');
      res.end();
...
async function getOrder(req, res, jwtBody) {
   ...
   console.log("hello in get order");
   let body = await new Promise(resolve => {
      let data = '';
      req.on('data', chunk => {
         data += chunk;
   ...
   console.log(orderToDisplay + "HEYYEYE");
...
//TODO fix the connection to use runQuery
...
async function checkStatus(req, res, jwtBody) {
   ...
   try {
      ...
      console.error("Error:", error.message);
      res.writeHead(500, {'Content-type': 'text/plain'});
      res.end("Error retrieving order status");
   }
}
...
async function getOrderID(req, res, jwtBody) {
   ...
   try {
      ...
      console.error("Error:", error.message);
      res.writeHead(500, {'Content-type': 'text/plain'});
      res.end("Error retrieving order ID of customer " + cusID + ".");
   }
}
...
async function getReward(req, res, jwtBody) {
   try {
      ...
      console.error("Error:", error.message);
      res.writeHead(500, {'Content-type': 'text/plain'});
      res.end("Error retrieving reward points of customer " + cusID + ".");
   }
}
...
async function redeem(req, res, jwtBody) {
   try {
      ...
      console.error("Error:", error.message);
      res.writeHead(500, {'Content-type': 'text/plain'});
      res.end("Error retrieving redeeming points of customer " + cusID + ".");
   }
...
async function checkDelivery(req, res, jwtBody) {
   try {
      ...
      let checkerText = await runQuery(customerPool, query);
      res.writeHead(200, {'Content-type': 'application/json'});
      res.end(JSON.stringify(checkerText));

   } catch (error) {
      console.error("Error:", error.message);
      res.writeHead(500, {'Content-type': 'text/plain'});
      res.end("Error checking if order is delivery or carryout for " + cusID + ".");
   }
}

async function stripeConfirmation(req, res) {
   console.log("HELFJOHUWFLIWFWHFOUHFWEOFI");
   let body = await new Promise(resolve => {
      let data = '';
      req.on('data', chunk => {
         data += chunk;
