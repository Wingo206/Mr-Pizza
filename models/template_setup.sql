...
foreign key(answered_by) references employee_account(eid) on delete set null);
...
foreign key(mid) references menu_item(mid) on delete cascade,
foreign key(reviewedBy) references customer_account(cid) on delete cascade);

create table made_by(order_id int, eid int, 
primary key(order_id, eid),
foreign key(order_id) references customer_order(order_id) on delete cascade,
foreign key(eid) references employee_account(eid) on delete cascade);
...
grant select, insert, update, delete, alter, drop on {{database}}.* to '{{sqlAdminUser}}'@'{{sqlHost}}';
